...
@migration
def add_competitor_tracking(conn):
    """Jim measures and quotes a pool, and the customer sometimes shows him a competitor's
    quote for the same job (or just names who else they're getting quotes from). This logs
    those competitor prices broken down by the same work_type categories QuoteCure already
    uses, so the rates are directly comparable to Jim's own -- not just a bottom-line total.
    competitor_quotes.total_price is a cached rollup of its items' prices (same convention as
    quotes.total_price), recomputed by _recalc_competitor_quote_total() in app.py whenever
    items change. saw_quote='N' covers the "customer just named them, didn't show the quote"
    case -- no items, total_price stays 0."""
    conn.execute('''CREATE TABLE IF NOT EXISTS competitors (
        competitor_id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        name TEXT NOT NULL UNIQUE,
        active TEXT DEFAULT 'Y'
    )''')
    conn.execute('''CREATE TABLE IF NOT EXISTS competitor_quotes (
        id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        customer_id INTEGER NOT NULL,
        competitor_id INTEGER NOT NULL,
        quote_id INTEGER,
        saw_quote TEXT DEFAULT 'Y',
        total_price REAL DEFAULT 0,
        notes TEXT DEFAULT '',
        created_by TEXT DEFAULT '',
        created_at TEXT DEFAULT (now()::text)
    )''')
    conn.execute('''CREATE TABLE IF NOT EXISTS competitor_quote_items (
        id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        competitor_quote_id INTEGER NOT NULL,
        work_type_id INTEGER,
        label TEXT DEFAULT '',
...
@migration
def add_quote_followup_templates(conn):
    """A/B-testable follow-up email templates for the 'quote sent -> wait N days -> webhook'
    GHL automation (see _ghl_webhook_quote_follow_up_due in app.py). Jim couldn't decide what
    the follow-up email should say, so instead of one fixed template QuoteCure picks a random
    *active* one per quote and remembers which one went out -- open-tracking pixels are
    unreliable (Gmail/Apple both defeat them by design), but "did this quote go on to get
    signed" is a real signal QuoteCure already has via quotes.signed_at, so that's what scores
    each variant. follow_up_sent_at also doubles as the idempotency guard, so a retried
    webhook can't send the same quote twice."""
    conn.execute('''CREATE TABLE IF NOT EXISTS quote_followup_templates (
        id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        label TEXT NOT NULL,
    ...
    cols = {r[1] for r in conn.execute("PRAGMA table_info(quotes)").fetchall()}
    if 'follow_up_template_id' not in cols:
        conn.execute("ALTER TABLE quotes ADD COLUMN follow_up_template_id INTEGER")
    if 'follow_up_sent_at' not in cols:
        conn.execute("ALTER TABLE quotes ADD COLUMN follow_up_sent_at TEXT DEFAULT ''")

    if not conn.execute("SELECT 1 FROM quote_followup_templates LIMIT 1").fetchone():
        starters = [
            ('Version A - direct', 'Following up on your quote',
             "Hi {name},\n\nJust following up on the quote we sent over -- wanted to check if "
             "you had any questions or were ready to move forward. Happy to hop on a call if "
             "that's easier.\n\n{company}"),
            ('Version B - low-pressure', 'Checking in',
             "Hi {name},\n\nNo rush at all -- just wanted to make sure the quote we sent landed "
             "okay and see if anything needs clarifying on our end.\n\n{company}"),
            ('Version C - value-forward', 'Following up on your pool project',
             "Hi {name},\n\nWanted to check back in on your quote. If it'd help, I'm happy to "
             "walk through the numbers or talk about financing options before you decide.\n\n{company}"),
        ]
        for label, subject, body in starters:
            conn.execute("INSERT INTO quote_followup_templates (label,subject,body_text) VALUES (?,?,?)",
                         (label, subject, body))


@migration
def add_ghl_lead_staging_and_opportunity_continuity(conn):
    """Phase 1 of the full pipeline automation (New Lead -> ... -> Won/Lost/Unqualified).
    Jim confirmed there is NO customers row at all until a GHL card reaches 'Qualified' --
    the New and Qualifying stages happen before any customer profile exists in QuoteCure, so
    they need their own lightweight staging table rather than living on customers. ghl_leads
    owns ghl_opportunity_id from the moment a lead is created, so one continuous GHL
    Opportunity/card can flow from New all the way through Won/Lost/Unqualified instead of a
    fresh one getting created at each stage (previously every quote got its own opportunity,
    with nothing at all tracked before a quote existed). When a lead converts to a real
    customer at Qualified, _get_or_create_customer_by_ghl_contact carries ghl_opportunity_id
    onto the new customers row and marks this staging row converted_customer_id/converted_at
    (kept, not deleted, for basic funnel-visibility later). nudge_1_sent_at/nudge_2_sent_at
    are the idempotency guards for the Qualifying 2-round nudge-then-auto-Unqualify cycle,
    same idiom as quotes.follow_up_sent_at.

    customers.ghl_opportunity_id/pipeline_stage mirror the same concept once a real customer
    exists -- pipeline_stage is a plain local label (not a raw GHL stage id), written by every
    outbound push (see _sync_quote_to_ghl and the future _sync_customer_to_ghl), driving the
    profile badge and idempotency checks without a live read back from GHL.

    new_lead_email_*/qualifying_nudge_email_* are single fixed templates (not the
    quote_followup_templates random-pick A/B pattern -- Jim didn't ask for variant-testing on
    these two), editable in Admin Settings same as financing_message."""
    conn.execute('''CREATE TABLE IF NOT EXISTS ghl_leads (
        id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        ...
        intake_email_sent_at TEXT DEFAULT '',
        nudge_1_sent_at TEXT DEFAULT '',
        nudge_2_sent_at TEXT DEFAULT '',
    ...
    if 'ghl_opportunity_id' not in cust_cols:
        conn.execute("ALTER TABLE customers ADD COLUMN ghl_opportunity_id TEXT DEFAULT ''")
    if 'pipeline_stage' not in cust_cols:
        conn.execute("ALTER TABLE customers ADD COLUMN pipeline_stage TEXT DEFAULT ''")

    settings_cols = {r[1] for r in conn.execute("PRAGMA table_info(company_settings)").fetchall()}
    for col in ('new_lead_email_subject', 'new_lead_email_body',
                'qualifying_nudge_email_subject', 'qualifying_nudge_email_body'):
        if col not in settings_cols:
            conn.execute(f"ALTER TABLE company_settings ADD COLUMN {col} TEXT DEFAULT ''")

    row = conn.execute("SELECT new_lead_email_subject FROM company_settings WHERE id=1").fetchone()
    if row and not row['new_lead_email_subject']:
        conn.execute(
            "UPDATE company_settings SET new_lead_email_subject=?, new_lead_email_body=?, "
            "qualifying_nudge_email_subject=?, qualifying_nudge_email_body=? WHERE id=1",
            ("Quick reply needed — let's get your pool project moving",
             "Hi {name},\n\nThanks for reaching out about your pool project -- we're excited to "
             "help bring your vision to life!\n\nTo help us hit the ground running, hit reply and "
             "let us know:\n\n  1. A few days/times that work for a quick call\n  2. A general "
             "idea of what you're thinking (new pool, full remodel, resurfacing, adding "
             "features, etc.)\n  3. A picture or two of your current pool or yard, if you've got "
             "them -- even rough phone photos help us get a head start before we talk\n\nJust "
             "reply to this email with whatever you've got -- even a few quick notes is a big "
             "help.\n\nTalk soon,\n{company}",
             "Following up on your pool project",
             "Hi {name},\n\nJust wanted to follow up on your pool project -- do you have a few "
             "minutes this week to chat about what you're looking for?\n\nReply here or give us "
...
@migration
def add_ledger_invoices(conn):
    """Jim: attach the actual sub invoices (usually 1-2) to each Ledger work item, so the
    cost entered has its receipt right there. Same base64-in-Postgres storage as
    customer_attachments; photos get shrunk on upload (see _shrink_image) since storage is
    the limit that matters on Render, and files only ever load on click, never on the page."""
    conn.execute('''CREATE TABLE IF NOT EXISTS ledger_invoices (
        id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        quote_id INTEGER NOT NULL,
        item_id INTEGER NOT NULL,
        source TEXT NOT NULL DEFAULT 'quote_line_item',
        filename TEXT NOT NULL,
        mime_type TEXT NOT NULL,
        file_data TEXT NOT NULL,
    ...
    conn.execute("CREATE INDEX IF NOT EXISTS idx_ledger_invoices_quote ON ledger_invoices (quote_id, item_id)")
